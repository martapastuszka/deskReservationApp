...
document.addEventListener('DOMContentLoaded', function(){
    ...
    //Get all desk elementsn
    var desks = svg.querySelectorAll('.desk');

    //Add click event listener to each desk
    desks.forEach(function(desk){
      console.log(desks.length);      // sztuczne użycie
      desk.addEventListener('click', function(e){
        openBookingModal(this.id);
      }, true);
    });
});
...
function openBookingModal(deskId) {
  ...
  // ustawiamy ID biurka w ukrytym input-cie i w nagłówku
    document.getElementById('desk_id').value = deskId;
    document.getElementById('modal-desk-id').textContent = deskId;
  ...
  //    // najpierw zdejmujemy ewentualny stary listener
  // bookBtn.replaceWith(bookBtn.cloneNode(true));
  // const freshBookBtn = document.getElementById('book-btn');
